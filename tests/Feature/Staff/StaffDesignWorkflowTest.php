...
class StaffDesignWorkflowTest extends TestCase
{
    ...
    public function test_assigned_designer_can_upload_private_source_and_customer_preview(): void
    {
        ...
        $this->assertNotEmpty($artwork->preview_files[0]['watermarked_path']);
        Storage::disk('local')->assertExists($artwork->preview_files[0]['watermarked_path']);

        $watermarkedPreview = $artwork->preview_files[0]['watermarked_path'];
        $this->assertStringEndsWith('.jpg', $watermarkedPreview);

        $sourceDimensions = getimagesize(Storage::disk('local')->path($artwork->preview_storage_path));
        $previewDimensions = getimagesize(Storage::disk('local')->path($watermarkedPreview));

        $this->assertNotFalse($sourceDimensions);
        $this->assertNotFalse($previewDimensions);
        $this->assertLessThan(
    ...
    public function test_assigned_designer_can_upload_multiple_source_and_preview_files_in_one_version(): void
    {
        ...
        $this->actingAs($designer)->post(
            ...
            [
                'source_artwork' => [
                    UploadedFile::fake()->create('front.psd', 100, 'image/vnd.adobe.photoshop'),
        ...
        $artwork = ArtworkVersion::where('design_job_id', $job->id)->sole();

        $this->assertCount(2, $artwork->source_files);
        $this->assertCount(2, $artwork->preview_files);
        $this->assertSame($artwork->storage_path, $artwork->source_files[0]['path']);
        $this->assertSame($artwork->preview_storage_path, $artwork->preview_files[0]['path']);

        foreach (array_merge($artwork->source_files, $artwork->preview_files) as $file) {
    ...
    public function test_other_designer_cannot_mark_assigned_job_ready(): void
    {
        ...
        app(CreateArtworkVersionService::class)->create(
            ...
            [
                ...
                'preview_storage_path' => 'artworks/test/preview.jpg',
    ...
    public function test_two_package_design_sides_can_be_marked_ready_independently(): void
    {
        ...
        foreach ([
            ...
            app(CreateArtworkVersionService::class)->create(
                $job->fresh(),
                [
                    'storage_path' => "artworks/{$job->side}/source.pdf",
                    'preview_storage_path' => "artworks/{$job->side}/preview.jpg",
                    'original_filename' => "{$job->side}.pdf",
                    'mime_type' => 'application/pdf',
                ],
                $designer
            );
    ...
    private function correctionRequestedJob(
        ...
        app(CreateArtworkVersionService::class)
            ->create(
                $job,
                [
                    'storage_path' => "artworks/{$job->side}/v1.psd",
                    'preview_storage_path' => "artworks/{$job->side}/v1-preview.jpg",
                    'original_filename' => "{$job->side}-v1.psd",
    ...
    private function confirmedOrder(
        ...
        $order = app(
            ...
            'side' => $packageCount === 1
                    ? 'LELAKI'
                    : null,
            'customer_name' => 'Staff Design Workflow Test',
        ]);
