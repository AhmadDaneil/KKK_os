    ...
    public function test_confirmed_order_cannot_be_confirmed_again(): void
{
    ...
    try {
        ...
        $this->fail('Confirmed order was allowed to be confirmed again.');
    ...
    $this->assertSame('DETAILS_CONFIRMED', $confirmed->status);
    $this->assertTrue(
        $originalConfirmedAt->equalTo($confirmed->details_confirmed_at)
    );
    $this->assertTrue(
        $originalConfirmationAt->equalTo($confirmed->confirmation->confirmed_at)
    );
    }
    public function test_final_review_contract_includes_complete_customer_review_data(): void
{
    ...
    $this->assertSame('Syafiq', $review['couple']['groom_abbreviation']);
    $this->assertSame('Nur Awanis', $review['couple']['bride_name']);
    $this->assertSame('Awanis', $review['couple']['bride_abbreviation']);

    $this->assertSame('Ahmad Firdaus', $review['second_couple']['groom_name']);
    $this->assertSame('Hajar', $review['second_couple']['bride_abbreviation']);

    $lelaki = collect($review['package_sides'])->firstWhere('side', 'LELAKI');
    $perempuan = collect($review['package_sides'])->firstWhere('side', 'PEREMPUAN');

    $this->assertNotNull($lelaki);
    $this->assertNotNull($perempuan);

    $this->assertSame('MINIMALIST', $lelaki['design']['theme']);
    $this->assertSame('L100', $lelaki['design']['design_code']);
    $this->assertSame('Walimatul Urus', $lelaki['design']['card_title']);
    $this->assertFalse($lelaki['design']['has_card_image']);

    $this->assertSame('Ahad', $lelaki['event']['day_name']);
    $this->assertSame('2026-12-20', $lelaki['event']['event_date']);
    $this->assertSame('10 Rejab 1448H', $lelaki['event']['hijri_date']);
    $this->assertSame('13:30', $lelaki['event']['bersanding_time']);
    $this->assertCount(3, $lelaki['event']['contacts']);

    $this->assertSame('SONGKET', $perempuan['design']['theme']);
    $this->assertSame('P200', $perempuan['design']['design_code']);
    $this->assertSame('Majlis Perkahwinan', $perempuan['design']['card_title']);
    $this->assertSame('Sabtu', $perempuan['event']['day_name']);
    $this->assertCount(3, $perempuan['event']['contacts']);

    $this->assertSame('COURIER', $review['fulfilment']['method']);
    $this->assertSame('Muhammad Syafiq', $review['fulfilment']['recipient_name']);
